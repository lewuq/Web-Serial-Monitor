...
function TelemetryImuPanel({ latest, detectedFields, zh, compact=false }: { latest:number[]; detectedFields:string[]; zh:boolean; compact?:boolean }) {
  ...
  return <article className={`sm-workspace-imu ${compact?'compact':''}`}>
    ...
    <div ref={bodyRef} className="sm-workspace-imu-body"><div className="sm-cube-scene sm-workspace-cube"><div className="sm-cube" style={{transform:`scale(${scale*fitScale}) rotateX(${-pitch}deg) rotateY(${yaw}deg) rotateZ(${roll}deg)`}}><div className="front">FRONT</div><div className="back">BACK</div><div className="right">X+</div><div className="left">X−</div><div className="top">Z+</div><div className="bottom">Z−</div></div><div className="sm-axis-key"><i className="x">X</i><i className="y">Y</i><i className="z">Z</i></div></div><div className="sm-euler-bindings">{['Roll','Pitch','Yaw'].map((axis,index)=><label key={axis}><span>{zh?['横滚','俯仰','偏航'][index]:axis}<b>{angles[index].toFixed(1)}°</b></span><select value={fields[index]} onChange={(event)=>setFields((current)=>current.map((field,fieldIndex)=>fieldIndex===index?Number(event.target.value):field))} disabled={!detectedFields.length}>{detectedFields.length?detectedFields.map((field,fieldIndex)=><option key={`${axis}-${field}`} value={fieldIndex}>{field}</option>):<option value={index}>{zh?'演示数据':'Demo data'}</option>}</select></label>)}</div></div>
...
function WorkspaceWidget({item,latest,detectedFields,zh,connected,onRemove,onSendPid}:{item:WorkspaceItem;latest:number[];detectedFields:string[];zh:boolean;connected:boolean;onRemove:()=>void;onSendPid:(p:number,i:number,d:number)=>Promise<void>}) {
  const [pressed,setPressed]=useState(false);
  const [enabled,setEnabled]=useState(true);
  const [setpoint,setSetpoint]=useState([64]);
  const [pid,setPid]=useState({p:[0.8],i:[0.15],d:[0.04]});
  const x=Number.isFinite(latest[0])?latest[0]:-2.08;
  const y=Number.isFinite(latest[1])?latest[1]:-5.4;
  return <article className={`sm-console-widget type-${item.type}`}><header><div><i><ComponentGlyph type={item.type}/></i><strong>{componentName(item.type,zh)}</strong></div><button className="sm-widget-remove" onClick={onRemove}>{zh?'移除':'Remove'}</button></header><div className="sm-console-widget-body">
    {item.type==='button'&&<div className="sm-button-stage"><Button className={pressed?'is-active':''} onClick={()=>setPressed((value)=>!value)}>{pressed?(zh?'输出已激活':'Output active'):(zh?'触发输出':'Trigger output')}</Button><span>{pressed?'HIGH':'LOW'}</span></div>}
    {item.type==='imu'&&<TelemetryImuPanel latest={latest} detectedFields={detectedFields} zh={zh} compact/>}
    {item.type==='dashboard'&&<div className="sm-control-metrics"><div><span>{zh?'温度':'Temperature'}</span><strong>{Math.abs(x*3+24).toFixed(1)} °C</strong></div><div><span>{zh?'总线电压':'Bus voltage'}</span><strong>{Math.abs(y/2+12).toFixed(2)} V</strong></div><div><span>{zh?'采样率':'Samples'}</span><strong>120 Hz</strong></div></div>}
    {item.type==='gauge'&&<GaugeDemo value={latest[3]??72} zh={zh}/>}
    {item.type==='status'&&<div className="sm-status-demo"><span className={enabled?'on':''}/><div><strong>{enabled?(zh?'设备已就绪':'Device ready'):(zh?'设备空闲':'Device idle')}</strong><small>{enabled?(zh?'正在接收遥测':'Receiving telemetry'):(zh?'输出已禁用':'Output disabled')}</small></div><Switch checked={enabled} onCheckedChange={setEnabled}/></div>}
    {item.type==='slider'&&<div className="sm-slider-demo"><div><span>{zh?'目标值':'Target'}</span><strong>{setpoint[0]??0}%</strong></div><Slider value={setpoint} onValueChange={(value)=>setSetpoint(typeof value==='number'?[value]:[...value])} min={0} max={100}/></div>}
    ...
    {item.type==='readout'&&<div className="sm-readout"><span>{detectedFields[0]??'accel.x'}</span><strong>{x.toFixed(6)}</strong><small>m/s²</small></div>}
  </div></article>;
}

function PlotWorkspaceWidget({points,plots,detectedFields,plotWindow,panOffset,verticalPan,plotPaused,resetKey,zh,onPanOffset,onVerticalPan,onTogglePlotPaused,onResetView,onRemove}:{points:DataPoint[];plots:PlotConfig[];detectedFields:string[];plotWindow:number;panOffset:number;verticalPan:number;plotPaused:boolean;resetKey:number;zh:boolean;onPanOffset:(value:number|((current:number)=>number))=>void;onVerticalPan:(value:number)=>void;onTogglePlotPaused:()=>void;onResetView:()=>void;onRemove:()=>void}) {
...
function TelemetryControlConsole({items,latest,points,plots,detectedFields,plotWindow,panOffset,verticalPan,plotPaused,resetKey,zh,connected,toolbarActions,onDrop,onRemove,onSendPid,onPanOffset,onVerticalPan,onTogglePlotPaused,onResetView}:{items:WorkspaceItem[];latest:number[];points:DataPoint[];plots:PlotConfig[];detectedFields:string[];plotWindow:number;panOffset:number;verticalPan:number;plotPaused:boolean;resetKey:number;zh:boolean;connected:boolean;toolbarActions:import('react').ReactNode;onDrop:(event:import('react').DragEvent<HTMLElement>)=>void;onRemove:(id:number)=>void;onSendPid:(p:number,i:number,d:number)=>Promise<void>;onPanOffset:(value:number|((current:number)=>number))=>void;onVerticalPan:(value:number)=>void;onTogglePlotPaused:()=>void;onResetView:()=>void}) {
  const renderItem=(item:WorkspaceItem)=>item.type==='plot'?<PlotWorkspaceWidget key={item.id} points={points} plots={plots} detectedFields={detectedFields} plotWindow={plotWindow} panOffset={panOffset} verticalPan={verticalPan} plotPaused={plotPaused} resetKey={resetKey} zh={zh} onPanOffset={onPanOffset} onVerticalPan={onVerticalPan} onTogglePlotPaused={onTogglePlotPaused} onResetView={onResetView} onRemove={()=>onRemove(item.id)}/>:<WorkspaceWidget key={item.id} item={item} latest={latest} detectedFields={detectedFields} zh={zh} connected={connected} onRemove={()=>onRemove(item.id)} onSendPid={onSendPid}/>;
...
export default function SerialWorkspace() {
  ...
  const stopDemo=useCallback((announce=true)=>{demoRunningRef.current=false;if(demoTimerRef.current!==null){window.clearInterval(demoTimerRef.current);demoTimerRef.current=null;}pendingLogsRef.current=pendingLogsRef.current.filter((line)=>line.direction!=='RX');pendingPointsRef.current=[];setDemo(false);if(announce)appendLog('SYS','Demo stream stopped.');},[appendLog]);
  const toggleDemo=()=>{if(demoRunningRef.current)return stopDemo();demoRunningRef.current=true;setPanOffset(0);setDemo(true);appendLog('SYS','Demo stream started.');demoTimerRef.current=window.setInterval(()=>{if(!demoRunningRef.current)return;const t=Date.now()/720;ingestLine(`accel(m/s^2) x=${(Math.sin(t)*8).toFixed(3)} y=${(Math.cos(t*.72)*7).toFixed(3)} z=${(8.6+Math.sin(t*.45)).toFixed(3)} gyro(rad/s) x=${(Math.sin(t*.4)*2).toFixed(3)} y=${(Math.cos(t*.55)*3).toFixed(3)} z=${(Math.sin(t*.8)*1.5).toFixed(3)} euler(deg) roll=${(Math.sin(t*.32)*55).toFixed(2)} pitch=${(Math.cos(t*.41)*35).toFixed(2)} yaw=${(((t*22)%360)-180).toFixed(2)}`,true);},120);};
  ...
  const sendPid=async(p:number,i:number,d:number)=>{const command=`PID P=${p.toFixed(3)} I=${i.toFixed(3)} D=${d.toFixed(3)}`;if(!portRef.current?.writable){setError(language==='zh'?'发送 PID 参数前请先连接串口设备。':'Connect a serial device before sending PID parameters.');return;}const payload=new TextEncoder().encode(`${command}\n`);const writer=portRef.current.writable.getWriter();try{await writer.write(payload);appendLog('TX',command,bytesToHex(payload));}catch(sendError){setError(sendError instanceof Error?sendError.message:'Could not send PID parameters.');}finally{writer.releaseLock();}};
