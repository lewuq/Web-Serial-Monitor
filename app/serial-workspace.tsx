...
function WorkspaceWidget({item,latest,detectedFields,zh,connected,onRemove,onSendPid}:{item:WorkspaceItem;latest:number[];detectedFields:string[];zh:boolean;connected:boolean;onRemove:()=>void;onSendPid:(p:number,i:number,d:number)=>Promise<void>}) {
  ...
  return <article className={`sm-console-widget type-${item.type}`}><header><div><i><ComponentGlyph type={item.type}/></i><strong>{componentName(item.type,zh)}</strong></div><button onClick={onRemove} aria-label={zh?`删除${componentName(item.type,zh)}`:`Remove ${componentName(item.type,zh)}`}><Trash2 size={13}/></button></header><div className="sm-console-widget-body">
...
function PlotWorkspaceWidget({points,plots,detectedFields,plotWindow,panOffset,verticalPan,zh,onPanOffset,onVerticalPan,onResetPlot,onRemove}:{points:DataPoint[];plots:PlotConfig[];detectedFields:string[];plotWindow:number;panOffset:number;verticalPan:number;zh:boolean;onPanOffset:(value:number|((current:number)=>number))=>void;onVerticalPan:(value:number)=>void;onResetPlot:()=>void;onRemove:()=>void}) {
...
export default function SerialWorkspace() {
  ...
  const resetPlotData=useCallback(()=>{pendingPointsRef.current=[];pointIndex.current=0;setPoints([]);setPlotWindow(120);setPanOffset(0);setVerticalPan(0);},[]);
  ...
  useEffect(()=>{const handlePageShow=(event:PageTransitionEvent)=>{if(event.persisted)resetPlotData();};window.addEventListener('pageshow',handlePageShow);return()=>window.removeEventListener('pageshow',handlePageShow);},[resetPlotData]);
