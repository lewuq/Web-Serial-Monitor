...
import { Activity, ArrowDownToLine, Braces, CircleStop, Cpu, Eraser, Gauge, GripHorizontal, GripVertical, Hand, Layers3, LineChart, Maximize2, Moon, MoveHorizontal, Pause, Play, Plug, Plus, Radio, RotateCcw, Send, SlidersHorizontal, Sun, TerminalSquare, Trash2, Unplug, Usb, ZoomIn, ZoomOut } from 'lucide-react';
...
function ComponentsGallery({ latest, detectedFields, zh }: { latest: number[]; detectedFields: string[]; zh: boolean }) {
  ...
  useEffect(()=>{const types=['button','imu','dashboard','gauge','status','slider','readout'];const cards=Array.from(galleryRef.current?.querySelectorAll<HTMLElement>('.sm-component-card')??[]);const cleanups=cards.map((card,index)=>{card.draggable=true;const handle=(event:DragEvent)=>{event.dataTransfer?.setData('application/x-web-serial-component',types[index]??'readout');if(event.dataTransfer)event.dataTransfer.effectAllowed='copy';};card.addEventListener('dragstart',handle);return()=>card.removeEventListener('dragstart',handle);});return()=>cleanups.forEach((cleanup)=>cleanup());},[]);
  ...
  const componentName=(type:string)=>({button:zh?'命令按钮':'Command button',imu:zh?'3D 姿态':'3D orientation',dashboard:zh?'遥测仪表盘':'Telemetry dashboard',gauge:zh?'径向仪表':'Radial gauge',status:zh?'状态指示器':'Status indicator',slider:zh?'设定值滑块':'Setpoint slider',readout:zh?'数值读数':'Numeric readout'}[type]??type);
...
export default function SerialWorkspace() {
  ...
  const appendLog = useCallback((direction:LogLine['direction'],text:string)=>{pendingLogsRef.current.push({id:Date.now()+Math.random(),time:now(),direction,text});if(pendingLogsRef.current.length>MAX_PENDING_LOGS)pendingLogsRef.current.splice(0,pendingLogsRef.current.length-MAX_PENDING_LOGS);},[]);
  const ingestLine = useCallback((line:string,showInTerminal=true)=>{const clean=line.trim();if(!clean)return;if(showInTerminal)appendLog('RX',clean);if(plotPausedRef.current)return;const parsed=parseSerialFields(clean);if(!parsed.values.length)return;if(detectedFieldsRef.current.join('\u0000')!==parsed.labels.join('\u0000')){detectedFieldsRef.current=parsed.labels;setDetectedFields(parsed.labels);setPlots((current)=>current.map((plot,index)=>({...plot,field:plot.field<parsed.labels.length?plot.field:Math.min(index,parsed.labels.length-1)})));}pointIndex.current+=1;pendingPointsRef.current.push({index:pointIndex.current,values:parsed.values});},[appendLog]);
  ...
  const readLoop = useCallback(async(port:BrowserSerialPort)=>{if(!port.readable)return;const reader=port.readable.getReader();readerRef.current=reader;const decoder=new TextDecoder();try{while(true){const{value,done}=await reader.read();if(done)break;if(!value)continue;setBytesReceived((total)=>total+value.byteLength);partialLine.current+=decoder.decode(value,{stream:true});const lines=partialLine.current.split(/\r\n|\n|\r/);partialLine.current=lines.pop()??'';lines.forEach((line)=>ingestLine(line,true));}}catch(readError){if(portRef.current)setError(readError instanceof Error?readError.message:'Serial read failed.');}finally{if(partialLine.current.trim())appendLog('RX',partialLine.current.trim());partialLine.current='';setLiveFragment('');reader.releaseLock();readerRef.current=null;}},[appendLog,ingestLine]);
  ...
  const sendMessage=async()=>{const text=message.trim();if(!text)return;if(!portRef.current?.writable)return setError('Connect a serial device before sending data.');const writer=portRef.current.writable.getWriter();try{await writer.write(new TextEncoder().encode(`${text}\n`));appendLog('TX',text);setMessage('');}finally{writer.releaseLock();}};
  ...
  const terminalPanel=<Tabs defaultValue="terminal" className="sm-terminal sm-card">
    <div className="sm-terminal-head"><TabsList><TabsTrigger value="terminal">{zh?'终端':'Terminal'}</TabsTrigger><TabsTrigger value="format">{zh?'数据格式':'Data format'}</TabsTrigger></TabsList><div className="sm-terminal-actions"><span className={connected||demo?'streaming':''}><i/>{terminalPaused?(zh?'输出已暂停':'Output paused'):connected||demo?(zh?'RX 实时':'RX live'):(zh?'RX 空闲':'RX idle')}</span><span><ArrowDownToLine size={14}/>{bytesReceived.toLocaleString()} {zh?'字节':'bytes'}</span><Button variant="ghost" size="sm" onClick={()=>setTerminalPaused((value)=>!value)}>{terminalPaused?<Play size={14}/>:<Pause size={14}/>} {terminalPaused?(zh?'继续':'Resume'):(zh?'暂停':'Pause')}</Button><Button variant="ghost" size="sm" onClick={clearTerminal}><Eraser size={14}/>{zh?'清空':'Clear'}</Button></div></div>
    <TabsContent value="terminal" className="sm-terminal-content"><div className="sm-output" ref={terminalRef} aria-live="polite" onScroll={(event)=>{const element=event.currentTarget;setAutoScroll(element.scrollHeight-element.scrollTop-element.clientHeight<28);}}>{logs.map((line)=><div className="sm-log" key={line.id}><time>{line.time}</time><b className={line.direction.toLowerCase()}>{line.direction}</b><code>{line.text}</code></div>)}{liveFragment&&<div className="sm-log sm-fragment"><time>{now()}</time><b className="rx">RX</b><code>{liveFragment}<span className="sm-caret"/></code></div>}{!logs.length&&!liveFragment&&<div className="sm-terminal-empty"><TerminalSquare size={20}/><span>{zh?'等待串口输出':'Waiting for serial output'}</span></div>}</div><div className="sm-terminal-footer"><button className={autoScroll?'active':''} onClick={()=>setAutoScroll((value)=>!value)}>{zh?'自动滚动':'Auto-scroll'} {autoScroll?(zh?'开启':'on'):(zh?'关闭':'off')}</button><span>{logs.length} {zh?'行已保留':'lines retained'}</span></div><div className="sm-send"><Input value={message} onChange={(event)=>setMessage(event.target.value)} onKeyDown={(event)=>event.key==='Enter'&&void sendMessage()} placeholder={zh?'输入命令并按 Enter':'Type a command and press Enter'} aria-label={zh?'串口命令':'Serial command'}/><Button onClick={sendMessage}><Send size={16}/>{zh?'发送':'Send'}</Button></div></TabsContent>
    <TabsContent value="format" className="sm-format">
      <div className="sm-format-intro"><div className="sm-component-icon"><Braces size={18}/></div><div><strong>{zh?'当前解析器真实支持的格式':'Formats the current parser actually supports'}</strong><p>{zh?'这不是 AI 猜测。解析器会逐行尝试文本 JSON、带标签的键值对，最后尝试分隔数值。只提取有限的整数或浮点数。':'This is deterministic parsing, not AI inference. Each completed text line is tried as JSON, labeled key/value text, then delimited numbers. Only finite integers and floating-point values are extracted.'}</p></div></div>
