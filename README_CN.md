...
| 版本 | 更新日期 | 主要内容 |
| --- | --- | --- |
| V0.6.0 | 2026-09-23 | 组件绑定改为使用实际检测到的字段和串口状态；命令组件支持配置并发送真实命令；改善串口写入与连接状态协调及 HEX 未完成片段显示；新增独立的清空绘图按钮和方向对应的三点缩放手柄。 |
| V0.5.2 | 2026-09-20 | 明确组件移除及 Demo/绘图控制的交互含义；绘图重置改为保留采样数据并恢复默认视图；优化 3D IMU 自适应尺寸，修复 Demo 按钮布局，并将暗夜模式设为默认主题。 |
